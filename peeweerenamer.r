...
SHOW_NAME<- "Pee Wee's Playhouse"
#----------------------------------------------------------------------
#pee wee tracks pasted from http://thetvdb.com/?tab=seasonall&id=71615&lid=7
trackpage<-read_html('http://thetvdb.com/?tab=seasonall&id=71615&lid=7')
                  
tracks<-html_node(trackpage,xpath='//*[@id="listtable"]')%>%html_table(header=TRUE)
names(tracks)<-c("Number","Name","Date","Image")
#remove specials

episodes<-filter(tracks,Number != "Special")%>%select(-Image)
#-------------------------------------------------------
# form new file names
newtitles<-data.frame(num=index(episodes),
                      newName=paste(SHOW_NAME, ".", episodes$Num,".",episodes$Name," (",episodes$Date,").mp4",
                                    sep = ""))
...
setwd("V:\\TV Shows\\Pee Wee's Playhouse")
list.files()
for (n in 1: nrow(episodes)) {
